...
void main(void)
{
   ...
   //gl_FragColor = sharp; //vec4( focus, focus, focus, 1.0); //mix(sharp, blur, clamp(range * abs(focus - sharp.a), 0.0, 1.0));
   //gl_FragColor = vec4( sharp.a, sharp.a, sharp.a, 1.0); //mix(sharp, blur, clamp(range * abs(focus - sharp.a), 0.0, 1.0));
   gl_FragColor = mix(sharp, blur, clamp(range * abs(focus - sharp.a), 0.0, 1.0));
}